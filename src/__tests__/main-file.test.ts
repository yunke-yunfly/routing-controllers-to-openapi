...
//   expect(genOpenapiv3FromRoutingControllers()).not.toBe(null);
// });
...
test('responseSchema参数不为对象对象', async () => {
  expect(() => verificationResponseSchema([])).toThrow(Error);
});

test('responseSchema.type 参数不为对象', async () => {
  expect(() =>
    verificationResponseSchema({
      type: 'array',
    }),
...
test('responseSchema.servers 值为真，但不是数组', async () => {
  expect(() => verificationServers({})).toThrow(Error);
});

test('responseSchema.servers 值为数组，但是空数组', async () => {
  expect(() => verificationServers([])).toThrow(Error);
});
